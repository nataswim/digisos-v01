...
<div class="container-fluid">
    ...
    <form method="POST" action="{{ route('editor.fiches.update', $fiche) }}">
        @csrf
        @method('PUT')
        ...
        <div class="row g-4">
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="long_description" 
                                      id="long_description" 
                                      class="d-none @error('long_description') is-invalid @enderror">{{ old('long_description', $fiche->long_description) }}</textarea>
                    ...
                    <div class="card-body p-4">
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" name="meta_title" id="meta_title" value="{{ old('meta_title', $fiche->meta_title) }}" class="form-control" maxlength="60">
                            </div>
                            <div class="col-md-6">
                                <label for="meta_keywords" class="form-label fw-semibold">Mots-clés</label>
                                <input type="text" name="meta_keywords" id="meta_keywords" value="{{ old('meta_keywords', $fiche->meta_keywords) }}" class="form-control">
                            </div>
                            <div class="col-12">
                                <label for="meta_description" class="form-label fw-semibold">Description SEO</label>
                                <textarea name="meta_description" id="meta_description" rows="3" class="form-control" maxlength="160">{{ old('meta_description', $fiche->meta_description) }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-4">
                    ...
                    <div class="card-body p-4">
                        <div class="mb-3">
                            ...
                            <select name="is_published" id="is_published" class="form-select">
                                <option value="0" {{ old('is_published', $fiche->is_published) == 0 ? 'selected' : '' }}>Brouillon</option>
                                <option value="1" {{ old('is_published', $fiche->is_published) == 1 ? 'selected' : '' }}>Publié</option>
                            </select>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="visibility" id="visibility" class="form-select">
                                <option value="public" {{ old('visibility', $fiche->visibility) === 'public' ? 'selected' : '' }}>Public</option>
                                <option value="authenticated" {{ old('visibility', $fiche->visibility) === 'authenticated' ? 'selected' : '' }}>Membres uniquement</option>
                            </select>
                        </div>

                        <div class="mb-3">
                            <label for="published_at" class="form-label fw-semibold">Date de publication</label>
                            <input type="datetime-local" name="published_at" id="published_at" value="{{ old('published_at', $fiche->published_at?->format('Y-m-d\TH:i')) }}" class="form-control">
                        </div>

                        <div class="mb-3">
                            <label for="sort_order" class="form-label fw-semibold">Ordre</label>
                            <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $fiche->sort_order) }}" class="form-control">
                        </div>

                        <div class="form-check mb-3">
                            <input type="checkbox" name="is_featured" id="is_featured" value="1" {{ old('is_featured', $fiche->is_featured) ? 'checked' : '' }} class="form-check-input">
                ...
                <div class="card border-0 shadow-sm mb-4">
                    ...
                    <div class="card-body p-4">
                        <div class="mb-3">
                            ...
                            <select name="fiches_category_id" id="fiches_category_id" class="form-select" required>
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {{ old('fiches_category_id', $fiche->fiches_category_id) == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="fiches_sous_category_id" id="fiches_sous_category_id" class="form-select">
                                <option value="">Aucune</option>
                                @if(isset($sousCategories))
                                    @foreach($sousCategories as $sousCategory)
                                        <option value="{{ $sousCategory->id }}" {{ old('fiches_sous_category_id', $fiche->fiches_sous_category_id) == $sousCategory->id ? 'selected' : '' }}>{{ $sousCategory->name }}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
                ...
                <div class="card border-0 shadow-sm mb-4">
                    ...
                    <div class="card-body p-4">
                        @if($fiche->image)
                            <img src="{{ $fiche->image }}" alt="{{ $fiche->title }}" class="img-fluid mb-3 rounded">
                        @endif
                        <div class="mb-3">
                            <div class="input-group">
                                <input type="text" name="image" id="image" value="{{ old('image', $fiche->image) }}" class="form-control">
                        ...
                        <div class="mt-3 {{ $fiche->image ? '' : 'd-none' }}" id="currentImagePreview">
                            <img id="imagePreview" src="{{ $fiche->image }}" class="img-fluid rounded shadow-sm" style="max-height: 150px;" alt="Aperçu">
                        </div>
                    </div>
                </div>
